...
export const REQUEST_URLS_TO_HIDE_RegEx = [/i\.clarity\.ms/]
...
Cypress.on('log:added', (ev) => {
  if (HIDE_ALL_REQUESTS ||
    (REQUEST_TYPES_TO_HIDE.includes(ev.displayName) && REQUEST_URLS_TO_HIDE_RegEx.some((reg) => reg.test(ev.consoleProps.URL)))) {
