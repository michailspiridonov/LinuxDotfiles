...
describe("Create an order with logged in user", () => {
	...
	before(() => {
		...
		cy.fixture("data/user/user_customer").then(customerFixture => customer = customerFixture);
	...
	it("Creates an order", () => {
		...
		CheckoutHandlers.extractOrderID(true);
		...
		cy.WaitForMagentoToLoad().then(() => {
			...
			AdminOrderHandlers.checkCustomerLoggedIn(customer);
