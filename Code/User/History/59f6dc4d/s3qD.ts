...
export class UserProfileHandlers {
	...
	static clearWishlist() {
		...
		cy.get("body").then(($body) => {
			if($body.text().includes("You have no items in your wish list.")) {
				return;
			} else {
				cy.get(UserProfile.wishlist.product_item).each(($el) => {
					cy.wrap($el).trigger("mouseover");
					cy.get(UserProfile.wishlist.remove_product_button).should("be.visible").click();
