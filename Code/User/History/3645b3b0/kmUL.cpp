...
size_t findShortestPath(const std::vector<std::pair<Place, Place>> & connections, Place start, Place end) {
  std::queue<Place> q;
...
std::list<Place> find_path(const Map &map) {
  const size_t SHORTEST_PATH_LEN = findShortestPath(map.connections, map.start, map.end);
  std::list<Place> path;
  //return empty list if no path found
  if (SHORTEST_PATH_LEN == std::numeric_limits<size_t>::max()) {
    return path;
  }
  // Return empty list if there is an undefined item
...
int main() {
  ...
  for (size_t i = 0; i < examples.size(); i++) {
    ...
    if (sol.size() != examples[i].first) {
      std::cout << "Wrong anwer for map " << i << " Path length: " << sol.size() << std::endl;
      fail++;
    }
  }
