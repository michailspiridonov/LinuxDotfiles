...
export class AdminFilterHandlers {
	/**
	...
	static filterTextByLabelText = (label: string, value: string) => {
		...
		cy.get(AdminFilter.open_filter_button).first().click({ force: true });
		AdminFilterHandlers.getFilterInputByLabelText(label).type(value);
		//apply filter
		cy.log("**applying filter**");
		cy.get(AdminFilter.apply_filter_button).click();
	};
	
	static filterSelectByLabelText = (label: string, value: string) => {
		//clear the filter if it is already set
		cy.log("**clearing filter**");
		cy.get("button").each($btn => {
			if ($btn.text() === "Clear all") {
		...
		cy.log("**opening filter**");
		cy.get(AdminFilter.open_filter_button).first().click({ force: true });
		AdminFilterHandlers.getFilterInputByLabelText(label).select(value);
		//apply filter
		cy.log("**applying filter**");
		cy.get(AdminFilter.apply_filter_button).click();
	};
