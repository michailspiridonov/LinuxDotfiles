...
export class CheckoutHandler {
  ...
  static decrementProduct (index: number): void {
    cy.log('**CheckoutHandler.decrementProduct**')
  ...
  static parsePrice (priceRaw: string): number {
    const priceParsed: number = parseFloat(priceRaw.replace(/,/, '.').replace(/\xa0/g, ''))
      ...
      if ($body.find(CheckoutLocators.address_verification.modal).length > 0) {
        cy.get(CheckoutLocators.address_verification.modal).then($modal => {
          if ($modal.find('input[type="radio"]').length > 0) {
            if (chooseSuggested) {
              cy.get(CheckoutLocators.address_verification.address_input).eq(1).check()
            } else {
              cy.get(CheckoutLocators.address_verification.address_input).eq(0).check()
            }
          }
        })
