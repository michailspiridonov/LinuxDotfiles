...
export class CheckoutHandler {
  ...
  static decrementProductQty (index: number): void {
    ...
    cy.getLoc(CheckoutLocators.cart.product.number_of_pieces_input).eq(index).invoke('text').then((qty) => {
      if (parseInt(qty) === 1) {
        cy.log('**AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA**')
        ...
        cy.log('**BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBS**')
        cy.getLoc(CheckoutLocators.cart.product.qty_minus).eq(index).click()
        cy.wait('@priceUpdate')
        CommonHandler.waitForFullLoad()
      }
