...
export class CheckoutHandler {
  ...
  static selectPaymentOption (option: string): void {
    ...
    cy.getLoc(CheckoutLocators.payment_methods).within(() => {
  ...
  static selectShipmentOption (option: string): void {
    ...
    cy.getLoc(CheckoutLocators.shipping_methods).within(() => {
  ...
  static parsePrice (priceRaw: string): number {
    const priceParsed: number = parseFloat(parseFloat(priceRaw.replace(/,/, '.').replace(/\xa0/g, '')).toPrecision(5))
