...
describe(`Create an order`, () => {
    ...
    it(`Creates an order`, () => {
        ...
        //including tax
        cy.get(`span#price-including-tax-product-price-3`).find(`span.price`).invoke('text').as('price_incl_tax');
        //excluding tax
        cy.get(`span#price-excluding-tax-product-price-3`).find(`span.price`).invoke('text').as('price_excl_tax');
        //proceed to checkout
        ...
        cy.get(`div#checkout-shipping-method-load`).find(`span.price`).invoke('text').as('shipping_price');
        //Continue the checkout
        cy.get(`button.continue`).click();
        //PAYMENT METHOD PAGE TESTS
        expect(this.price_incl_tax).to.be.a('string');
        // checkPaymentPage(customer, this.price_incl_tax, this.price_excl_tax, this.shipping_price);
        //TODO
        //place order
