...
describe(`Create an order`, () => {
    let customer;
    let admin;
    let order_id: string = `0000000010`;
    let price_incl_tax;
    let price_excl_tax;
    let shipping_price;
    const PRODUCT = `test`;
    function getProductPriceIncl($el) {
        price_incl_tax = $el.text();
    }

    function getProductPriceExcl($el) {
        price_excl_tax = $el.text();
    }
    before(() => {
    ...
    it(`Creates an order`, () => {
        ...
        cy.get(`span#price-including-tax-product-price-3`).find(`span.price`).then(($price) => {
            getProductPriceIncl($price);
            cy.get(`span.price-including-tax`).find(`span.minicart-price`).should(`contain`, price_incl_tax);

        });
        //excluding tax
        cy.get(`span#price-excluding-tax-product-price-3`).find(`span.price`).then(($price) => {
            getProductPriceExcl($price);
            cy.get(`span.price-excluding-tax`).find(`span.minicart-price`).should(`contain`, price_excl_tax);
        });
        //proceed to checkout
        ...
        checkPaymentPage(customer, price_incl_tax, price_excl_tax, shipping_price);
        //TODO
        //place order
    ...
    it(`Checks the order in the admin panel`, () => {
        //reqest intercepted to wait for the side menu to load
