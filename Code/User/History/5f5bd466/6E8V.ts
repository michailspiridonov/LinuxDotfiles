import { LoginScreenLocators, randomString } from "../index";
...
function getTokens() {
  cy.request({
    ...
    body: {
      ...
      code: "4/0AdQt8qioiBM9HhA5Mej01f0d2XtzY361w63fc6z3ZLsCXjzpZHNxQELfhMwlpAujqttJfg",
    },
  });
}
class reglink {
...
function getEmailLink() {
  cy.request({
    ...
    headers: {
      Authorization:
        "Bearer ya29.A0AVA9y1uzjGJO1ThaRnn2kbvfs4qCTmPoqwdeTrFNi49Kv-ys532lY5vCDFUEvejtxP8QpXJBIDGq3s2pORDaV_q7tLTsIIAaZ0u5h1I8zEcXGGRSoA1Dq7mVVAuaGCKMywWBA2QesonS1iwGRJ0SKROCkwVOYUNnWUtBVEFTQVRBU0ZRRTY1ZHI4SXljUV9KMV8yZ2xLS3N5WU9BYnRMdw0163",
    },
  });
}
...
describe("Create new user", () => {
  ...
  it("creates a new user", () => {
    ...
    cy.get(LoginScreenLocators.registation.primaryEmail).type(
      Cypress.env("ATemailName") +
        "+" +
        randomString +
        "@" +
        Cypress.env("ATemailDomain")
    );
    cy.get(LoginScreenLocators.registation.emailRadioSelect).click();
    cy.get(LoginScreenLocators.registation.verificationEmail).type(
      "johndoe@example.com"
    );
    ...
    cy.origin("google.com", { args: { link } }, ({ link }) => {
      ...
      //fetch the email
      cy.wait(10000);
      cy.get("div.list-sender")
        .contains("onboarding@capitain-io-demo.compacer.net")
        .eq(0)
        .click();
      cy.get(".button > tbody > tr > td")
        .find("a")
        .invoke("attr", "href")
        .then(($href) => {
          console.log(link);
