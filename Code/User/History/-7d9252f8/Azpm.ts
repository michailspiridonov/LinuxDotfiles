...
export class ApiHandler {
  ...
  static makeRequest2 (requestOptions: Partial<Cypress.RequestOptions>, remainingRepeats: number = 5): Cypress.Chainable<Cypress.Response<any>> {
    ...
    return cy.waitUntil((requestInfo) => {
      return cy
        .request(requestOptions)
        .then(res => {
          const reqUrlInfo = `url: ${requestOptions.url}`
          const reqBodyInfo = `body: ${requestOptions.body ? JSON.stringify(requestOptions.body).replace(/\\n|\s+(?=\s)/g, '') : undefined}`
          const resInfo = `${JSON.stringify(res.body)}`
          requestInfo.reqUrlInfo = `url: ${requestOptions.url}`
          requestInfo.reqBodyInfo = `body: ${requestOptions.body ? JSON.stringify(requestOptions.body).replace(/\\n|\s+(?=\s)/g, '') : undefined}`
          requestInfo.resInfo = `${JSON.stringify(res.body)}`
      ...
      timeout: 1000,
      errorMsg: `_request_\n${requestInfo.reqUrlInfo}\n${requestInfo.reqBodyInfo}\n_response_\n${requestInfo.resInfo}`
    })
  }
}
