...
describe('User Account - My Data Tab', () => {
  ...
  if (!['cz'].includes(CONFIG_CONST.COUNTRY)) {
    it('9318 - Visuals', { tags: [TAG.PROD_FRIENDLY, TAG.P2, TAG.DEVCLOUD] }, () => {
      const name = new RegExp(CONFIG_CONST.DATA.user.regex_validation.name)
      ...
      cy.getLocScroll(UserLocators.account_info.fullname).invoke('text').should('match', regEx.name)
