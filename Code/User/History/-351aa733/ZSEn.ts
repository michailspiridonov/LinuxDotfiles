...
describe('Product Lists', () => {
  ...
  it('6122 - List of products in category', { tags: [TAG.PROD_FRIENDLY, TAG.P1, TAG.DEVCLOUD] }, () => {
    ...
    ElasticsearchApiHandler.getProductData(productESBuilder)
      .then(products => {
        const productsWithCategory = products.filter(function (product: { category: any }) {
          if (product.category) {
            return product
          }
          return null
        })
        return Cypress._.sample(productsWithCategory)
      })
      .then(product => {
  ...
  it('6118 - Product detail', { tags: [TAG.PROD_FRIENDLY, TAG.DEVCLOUD, TAG.P1, TAG.DESKTOP] }, () => {
  ...
  if (['cz'].includes(CONFIG_CONST.COUNTRY)) {
    // skipped because of https://dev.azure.com/drmaxglobal/catalog-team/_workitems/edit/7567, redesign needed
    it.skip('7555 - Product line', { tags: [TAG.PROD_FRIENDLY, TAG.CZ, TAG.P1] }, () => {
    ...
    it('7864 - Product list on actions and sales page', { tags: [TAG.PROD_FRIENDLY, TAG.CZ, TAG.P1, TAG.DEVCLOUD, TAG.DESKTOP] }, () => {
