...
export class AdminProductApiHandlers {
	...
	/**
	 * Removes non searchable products from an array of products
	 * @param {Product[]} products array of products
	 * @returns {Product[]} array of products with only searchable products
	 */
	static removeNonSearchableProducts(products: Product[]) {
		const searchableProducts: Product[] = [];
		products.forEach(product => {
			if (product.visibility === 4) {
				searchableProducts.push(product);
			}
		});
