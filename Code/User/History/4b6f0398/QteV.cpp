...
std::list<Place> find_path(const Map &map) {
  std::list<Place> path;
  // add the reversed connections to the graph
  for (const auto &c : map.connections) {
    map.connections.emplace_back(c.second, c.first);
  }
  ...
  while (!q.empty()) {
    ...
    if (std::all_of(collected.begin(), collected.end(), [](bool b) { return b; })) {
      // am I at the end?
      if (current == map.end) {
      ...
      // save the path
      std::list<Place> p = path;
      // clear visited places and go to the end
      visited.clear();
      q.push(current);
      ...
      while (!q.empty()) {
        ...
        q.pop();
        for (const auto &c : map.connections) {
          if (c.first == current && visited.find(c.second) == visited.end()) {
            visited.insert(c.second);
            q.push(c.second);
            prev[c.second] = current;
          }
        }
      }
    }
