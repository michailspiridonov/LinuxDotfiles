...
const checkEmailToken = (emailSubject) => {
  cy.log('**Waiting for an email, might take a while.**')
  cy.wait(20 * 1000)
  cy.mailosaurGetMessage(serverId, cy.get('.MuiGrid-grid-xs-10 > .MuiBox-root > [style="position: relative;"] > .MuiFormControl-root > .MuiInputBase-root > .MuiInputBase-input'){
	...
	it('', () => {
		...
		checkEmailToken();
		cy.get('.MuiGrid-grid-xs-10 > .MuiBox-root > [style="position: relative;"] > .MuiFormControl-root > .MuiInputBase-root > .MuiInputBase-input').type(this.token)
