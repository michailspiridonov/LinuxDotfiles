...
describe(`Create an order`, () => {
    ...
    it(`Creates an order`, () => {
        ...
        cy.visit(orders).then(() => {
            cy.log(`**logging local storage**`);
            cy.log(`**local storage len:${localStorage.length}**`);
            for(var i = 0; i < localStorage.length; i++){
                cy.log(`LOCAL STORAGE ITEM: ${localStorage.key(i)}\n`);
                cy.log(`====================\n`);
                cy.log(localStorage.getItem(localStorage.key(i)));
                cy.log(`====================\n`);
                if(localStorage.key(i) == "order_id"){
                    order_id = localStorage.getItem(localStorage.key(i));
                    cy.log(`**order_id: ${order_id}**`);
                }
            }
        });
