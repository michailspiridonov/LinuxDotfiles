...
if (CONFIG_CONST.ENV_TYPE === 'stage') {
  describe('E2E', () => {
    ...
    it('7036 - Not logged-in user order via API', { tags: [TAG.E2E] }, () => {
      ...
      AdminRequestHandler.createBearerToken().then(authToken => {
        cy.get('@orderNumber').then((incrementIdRaw) => {
          ...
          AdminRequestHandler.getOrderDetail(incrementId, authToken).then(response => {
            ...
            cy.get('@productSKU').then((productSKU) => {
    ...
    it('7039 - Logged-in user order via API', { tags: [TAG.E2E] }, () => {
      ...
      cy.getLoc(UserLocators.login_alert).should('not.exist')
    ...
    it('7060 - Not logged-in user order', { tags: [TAG.E2E] }, () => {
      ...
      CheckoutHandler.selectPaymentOption(CONFIG_CONST.DATA.default_payment_method) // TODO selector
      ...
      cy.get('body').then(body => {
        if (body.find('.modal').length > 0) {
          cy.get('.modal .address input[type="radio"]').eq(0).click()
    ...
    it('7061 - Logged-in user order', { tags: [TAG.E2E] }, () => {
      const timestamp = Date.now()
      cy.visit(CONFIG_CONST.HOMEPAGE_URL)
      CommonHandler.waitForFullLoad()
      HomepageHandler.goToUserLogin()
