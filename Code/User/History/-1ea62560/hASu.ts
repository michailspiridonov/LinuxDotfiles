...
describe('E2E', { retries: 0 }, () => {
  ...
  it.only('7060 - Not logged-in user order', { tags: [TAG.E2E, TAG.P1] }, () => {
    const timestamp = Date.now()
    ElasticsearchApiHandler.getProductData(elasticBuilder).then((products) => {
      const product = Cypress._.sample(products)
      cy.wrap(product).as('product').then($product => console.log(CONFIG_CONST.HOMEPAGE_URL + $product.url_path))
