...
export abstract class ElasticSearchRequestBuilder {
  ...
  public build (): string {
    ...
    const elasticPrefix = (['it', 'ssb'].includes(CONFIG_CONST.COUNTRY) || (Cypress.env('environment') === 'cz_upgrade_stage')) ? `_e?index=drmax_frontend_${this.type}_1&body=` : `_e?type=${this.type}&body=`
